...
cytokines_adjusted <- cytokines %>% 
  ...
  mutate(values = if_else(tissue == "ileum", values/tissue_weight_mg, values)) %>%
  pivot_wider(
    id_cols = c(
...
#pivot_wider(id_cols = c(sample_id,exp_number, group, received_antibiotics,maternal_treatment, #tissue,tissue_weight_mg, adjusted_values), names_from = variables, values_from = values)
```
...
jitter_plot <- cytokines_long %>% 
  ...
  #filter(variables != "INFg") %>% 
  drop_na(adjusted_values) %>% 
  ggplot(aes(x = group, y = adjusted_values, color = group)) +
  geom_jitter() +
  facet_wrap(~ variables,
             scales = "free") 

...
cytokines <- cytokines %>% 
  ...
  mutate(values = if_else(tissue == "Illeum", values/tissue_weight_mg, values)) %>% 
  pivot_wider(id_cols = c(sample,group,tissue,tissue_weight_mg, MAT,FEED), names_from = variables, values_from = values)
```

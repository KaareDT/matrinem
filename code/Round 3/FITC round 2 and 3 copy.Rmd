---
title: "FITC analysis script - round 2 and 3"
author: "Kaare"
date: "`r Sys.Date()`"
...
```{r}
tbl1 <- fitc %>% 
  filter(exp_number == 2 | exp_number == 3) %>% 
  mutate(Log10_values = log10(values)) %>% 
  select(group2, values, Log10_values) %>% 
...
tbl1_gt <- tbl1 %>% 
 as_gt() %>%
  ...
  tab_source_note(
    source_note = md("**Matrinem round 3**")
  ) %>%
...
ggplot(aes(x = group2, 
  ...
  stat_compare_means(comparisons = my_comparisons, label.y = c(30, 30, 30, 50), method = "wilcox.test")+
...
ggplot(aes(x = group2, 
  ...
  geom_point() +
  #coord_cartesian(ylim = c(1.5, 16)) + 
  stat_compare_means(comparisons = my_comparisons, label.y = c(1.5, 1.5, 1.5, 1.7), method = "wilcox.test")+
  # stat_compare_means(aes(label = paste0("Kruskal-Wallis \n p = ", ..p.format..)),
  #   label.y.npc = 0.75,
  #   label.x.npc = 0.55) +
  labs( # Change title, and axis names
